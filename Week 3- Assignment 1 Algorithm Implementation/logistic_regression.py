...
class LogisticRegression:
    ...
    def predict(self, X):
        ...
        y_predicted_labels = np.where(y_predicted_probs > 0.5, 1, 0)
...
class SimpleNeuralNetwork:
    ...
    def fit(self, X, y, n_iters=1000):
        ...
        for _ in range(n_iters):
            ...
            self.weights_ho += np.dot(hidden_layer_output.T, d_predicted_output) * self.lr / n_samples
            self.bias_o += np.sum(d_predicted_output, axis=0, keepdims=True) * self.lr / n_samples
            self.weights_ih += np.dot(X.T, d_hidden_layer) * self.lr / n_samples
            self.bias_h += np.sum(d_hidden_layer, axis=0, keepdims=True) * self.lr / n_samples
    ...
    def predict(self, X):
        ...
        # Convert probabilities to class labels
        predicted_labels = np.where(predicted_output_probs.flatten() > 0.5, 1, 0)
...
hidden_nodes = 45 # Adjusted for a higher number of features (86)
